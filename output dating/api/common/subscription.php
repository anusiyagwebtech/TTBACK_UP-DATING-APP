...
if($checknum>0)
{ 
$stmt = $db->prepare("SELECT * FROM `subscription` WHERE `status`='1' ORDER BY `id` DESC ");	
...
extract($row);
    ...
    $ps_item1[]=array(
            "subscriptionId"=>$id,"subscriptionName"=>$name,"subscriptionDesc"=>$desc,
            "subscriptionAmount"=>$amount,
            "subscriptionValidity"=>$validity." Days",
        );
 ...
 if(count($ps_item1)>0) { 
  ...
  echo json_encode(
        array("success" => "true","error" => "false","subscription" => $ps_item1)
    );
   }
   else
   {
