...
$stmt = $db->prepare("SELECT * FROM `likes` WHERE `fromid`='".$userrow['id']."' ORDER BY `id` DESC ");	
...
if(getuser('photo',$toid)!=''){
    $cimg=getuser('photo',$toid);
}
else
{
   $cimg=''; 
...
$timeduration=timedifference($fromtime,$totime);
    ...
    $ps_item1[]=array(
            "userid"=>$toid,
            "photo"=>$cimg,
            "username"=>nulltoempty(getuser('username',$toid)),
            "age"=>nulltoempty(getuser('age',$toid)),
            "time"=>$timeduration
        );
    $count++;
}
