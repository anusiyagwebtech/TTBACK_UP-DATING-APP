...
$dstmt11 = $db->prepare("SELECT * FROM `driver` WHERE `token`='".$token."' ");	

$dstmt11->execute();
$dchecknum11 = $dstmt11->rowCount();

if($checknum11>0 || $dchecknum11>0)
{
$rres=$stmt11->fetch(PDO::FETCH_ASSOC);
	
	
	
$stmt = $db->prepare("SELECT * FROM `faq` ORDER BY `id` DESC ");	
...
extract($row);
 ...
 if(count($ps_item1)>0) { 
  ...
  echo json_encode(
        array("success" => "true","error" => "false","faq_list" => $ps_item1)
    );
   }
   else
   {
