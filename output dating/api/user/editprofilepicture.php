...
if(
    !empty($data->userid) && !empty($data->user_profile_image) 
){
...
{
    $filename = time() . ".jpg";
    $file = fopen('../../images/profile/'. $filename, 'wb');
    $binary = base64_decode($image);
   // header('Content-Type: bitmap; charset=utf-8');
    $file = fopen('../../images/profile/'.$filename, 'wb');
    fwrite($file, $binary);
    fclose($file);
    $photo=$sitename.'images/profile/'.$filename;
}    

$query = "UPDATE `register` SET
                    photo='".$photo."' WHERE id='".$data->userid."'";
...
$stmt->execute();
  // set response code - 400 bad request
    ...
    // tell the user
    echo json_encode(array("success" => "true", "error" => "false","userid"=>$data->userid, "message" => "Photo Updated Successfully"));
}
