...
// echo $_REQUEST['id'].'<br>';
// echo $_REQUEST['table'].'<br>';
// echo $_REQUEST['path'].'<br>';
// echo $_REQUEST['images'].'<br>';
// echo $_REQUEST['pid'];
// echo $_REQUEST['path'];
...
//  echo $_REQUEST['path'] . $_REQUEST['image'];
// // exit;
    if ($_REQUEST['table'] != '') {
        // $pimg = explode(",", getproduct('image', $_REQUEST['id']));
        // $pimg =  getproduct('image', $_REQUEST['id']);   


        if (($key = array_search($_REQUEST['image'], $pimg))) {
            unset($pimg[$key]);
        }
        // unlink("../images/brand/" . $_REQUEST['image']); 
        // unlink("../../images/product/models/" . $_REQUEST['image']);
    ...
    $htry = $db->prepare("INSERT INTO `admin_history` (`admin_uid`,`ip`,`checkintime`,`checkouttime`) VALUES (?,?,?,?)");
