...
function companylogin($a) {
    ...
    /* $getlogo = mysql_fetch_array(mysql_query("SELECT * FROM `profile_area` WHERE `pid`='" . $_SESSION['GRUID'] . "'"));
    ...
    /* global $db;
      ...
      $get1->execute(array($_SESSION['GRUID']));
...
function adddepartment($a = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['did'] == '') {
            ...
            $resa->execute(array(strtolower(trim($a)), 0, $c, $d, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Department', 27, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['did'] == '') {
            ...
            $resa->execute(array(strtolower(trim($a)), 0, $c, $d, $_SESSION['GRUID'], $e));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Department', 27, 'Update', $_SESSION['GRUID'], $d, $e));
...
function deldepartment($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Department', 1, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));

        $get = $db->prepare("DELETE FROM `department` WHERE `did` =? ");

        $get->execute(array($c));

        //$htry = mysql_query("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES ('Department','1','Delete','" . $_SESSION['GRUID'] . "','" . $_SERVER['REMOTE_ADDR'] . "','" . $c . "')");
...
function addsupplier_group($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['SgID'] == '') {
            ...
            $resa->execute(array(trim($a), $b, $c, $d, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Supplier Group', 2, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['SgID'] == '') {
            ...
            $resa->execute(array(trim($a), $b, $c, $d, $_SESSION['GRUID'], $e));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Supplier Group', 2, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delsuppliergroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Supplier Group', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addsupplier($a = '', $b = '', $c = '', $d = '', $e = '', $f = '', $g = '', $h = '', $i = '', $j = '', $k = '') {
    ...
    if ($k == '') {
        ...
        if ($ress['SupID'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $e, $f, $g, $h, $i, $j, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Supplier', 3, 'Insert', $_SESSION['GRUID'], $j, $insert_id));
        ...
        if ($ress['SupID'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $e, $f, $g, $h, $i, $j, $_SESSION['GRUID'], $k));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Supplier', 3, 'Update', $_SESSION['GRUID'], $j, $k));
...
function delsupplier($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Supplier', 3, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function additem_group($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['ItemID'] == '') {
            ...
            $htry->execute(array('Item Group', 2, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['ItemID'] == '') {
            ...
            $htry->execute(array('Item Group', 2, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delitemgroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Item Group', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function additem_type($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['ItemID'] == '') {
            ...
            $htry->execute(array('Item Type', 2, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['ItemID'] == '') {
            ...
            $htry->execute(array('Item Type', 2, 'Update', $_SESSION['GRUID'], $d, $e));
...
function addmanufacturer($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['MID'] == '') {
            ...
            $htry->execute(array('Manufacurer', 2, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['MID'] == '') {
            ...
            $htry->execute(array('Manufacturer', 2, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delmanufacturer($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Manufacturer', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addshipping_type($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['ShipID'] == '') {
            ...
            $htry->execute(array('Shipping Type', 2, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['ItemID'] == '') {
            ...
            $htry->execute(array('Shipping Type', 2, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delshippingtype($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Shipping Group', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function adddesignation($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['id'] == '') {
            ...
            $resa->execute(array($a, strtolower(trim($b)), $c, $d, $_SESSION['GRUID']));
            ...
            $htry->execute(array('Designation', 28, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['id'] == '') {
            ...
            $resa->execute(array(trim($a), trim($b), $c, $d, $_SESSION['GRUID'], $e));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Designation', '28', 'Update', $_SESSION['GRUID'], $d, $e));
...
function deldesignation($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Designation', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addwarehouse($a = '', $b = '', $c = '', $d = '', $e = '', $f = '', $g = '') {
    ...
    if ($g == '') {
        ...
        if ($ress['WhID'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $e, $f, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Warehouse', 10, 'Insert', $_SESSION['GRUID'], $f, $insert_id));
        ...
        if ($ress['WhID'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $e, $f, $_SESSION['GRUID'], $g));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Warehouse', 10, 'Update', $_SESSION['GRUID'], $f, $g));
...
function delwarehouse($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Warehouse', 10, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addpartnertype($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['Partner_id'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Partner Type', 13, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['PID'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $_SESSION['GRUID'], $e));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Partner Type', 13, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delpartnertype($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Partner Type', 13, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addlanguage($a = '', $b = '', $c = '', $d = '', $e = '', $f = '') {
    ...
    if ($f == '') {
        ...
        if ($ress['Language_id'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $e, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('language', 13, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['Language_id'] == '') {
            ...
            $resa->execute(array($a, $b, $c, $d, $e, $_SESSION['GRUID'], $f));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Language', 13, 'Update', $_SESSION['GRUID'], $e, $f));
...
function dellanguage($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Language', 14, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addproperties($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['PID'] == '') {
            ...
            $htry->execute(array('properties', 11, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['PID'] == '') {
            ...
            $htry->execute(array('properties', 11, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delproperties($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('properties', 11, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addindustry($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['InID'] == '') {
            ...
            $htry->execute(array('Industry', 2, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['InD'] == '') {
            ...
            $htry->execute(array('Industry', 2, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delindustry($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Industry', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addpaymentterm($a = '', $b = '', $c = '', $d = '', $e = '') {
    ...
    if ($e == '') {
        ...
        if ($ress['InID'] == '') {
            ...
            $htry->execute(array('PaymentTerm', 2, 'Insert', $_SESSION['GRUID'], $d, $insert_id));
        ...
        if ($ress['PID'] == '') {
            ...
            $htry->execute(array('PaymentTerm', 2, 'Update', $_SESSION['GRUID'], $d, $e));
...
function delpaymentterm($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('PaymentTerm', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
...
function addchannel($channel_name, $ch_code, $description, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['ChID'] == '') {
            ...
            $resa->execute(array($ch_code, $channel_name, $description, $order, $status, $ip, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Channel', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['ChID'] == '') {
            ...
            $resa->execute(array($ch_code, $channel_name, $description, $order, $status, $ip, $_SESSION['GRUID'], $id));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('channel', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delchannel($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Channel', 3, 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addemplpyeegroup($group_name, $group_code, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['GrID'] == '') {
            ...
            $resa->execute(array($group_name, $group_code, $order, $status, $ip, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Employee Group', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['GrID'] == '') {
            ...
            $resa->execute(array($group_name, $group_code, $order, $status, $ip, $_SESSION['GRUID'], $id));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Employee Group', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delemplpyeegroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Employee Group', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addemployee($empname, $empcode, $enroll, $title, $first_name, $middle_name, $last_name, $dobdate, $gender, $designation, $department, $hiredon, $terminate, $terminatereason, $image, $address, $country, $state, $city, $area, $postal, $address1, $country1, $state1, $city1, $area1, $postal1, $phone, $mobile, $emailid, $website, $description, $workshift, $salaryfre, $emptype, $isdailywages, $grossamount, $grosspercetage, $basicsalary, $mrppackprice, $maxleaveallowed, $overtimerate, $maxexpenalloed, $salaryaccthead, $casleavealloed, $ourbank, $bankacctname, $bankacctnumber, $bankaccttype, $micrcode, $ifsccode, $incenttivepln, $conemail, $conpassword, $conserver, $conport, $enableserver, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['EmpID'] == '') {
            ...
            $htry->execute(array('Employee', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['EmpID'] == '') {
            ...
            $htry->execute(array('Employee', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delemployee($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Employee', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addbrand($menuf, $brand_name, $brand_code, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['BrID'] == '') {
            ...
            $resa->execute(array($menuf, $brand_name, $brand_code, $order, $status, $ip, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Brand', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['BrID'] == '') {
            ...
            $resa->execute(array($menuf, $brand_name, $brand_code, $order, $status, $ip, $_SESSION['GRUID'], $id));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Brand', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delbrand($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Brand', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
//        if ($ress['ItSubID'] == '') {
        ...
        $resa->execute(array($group_name, $subgroup, $order, $status, $ip, $_SESSION['GRUID']));

        $insert_id = $db->lastInsertId();

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Sub Group', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
...
//        if ($ress['ItSubID'] == '') {
        ...
        $resa->execute(array($group_name, $subgroup, $order, $status, $ip, $_SESSION['GRUID'], $id));

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Sub Group', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delsubgroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Sub Group', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
//        if ($ress['ItinnerID'] == '') {
        ...
        $resa->execute(array($group_name, $subgroup, $innergroup, $order, $status, $ip, $_SESSION['GRUID']));

        $insert_id = $db->lastInsertId();

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Inner Group', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
...
//        if ($ress['ItinnerID'] == '') {
        ...
        $resa->execute(array($group_name, $subgroup, $innergroup, $order, $status, $ip, $_SESSION['GRUID'], $id));

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Inner Group', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delinnergroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Inner Group', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function adduom($name, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['UomID'] == '') {
            ...
            $resa->execute(array($name, $order, $status, $ip, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('UOM', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['UomID'] == '') {
            ...
            $resa->execute(array($name, $order, $status, $ip, $_SESSION['GRUID'], $id));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('UOM', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function deluom($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('UOM', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addmargin($name, $margincode, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['MaID'] == '') {
            ...
            $resa->execute(array($name, $margincode, $order, $status, $ip, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Margin', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['MaID'] == '') {
            ...
            $resa->execute(array($name, $margincode, $order, $status, $ip, $_SESSION['GRUID'], $id));

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Margin', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delmargin($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Margin', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addcusgroup($name, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['CuGID'] == '') {
            ...
            $htry->execute(array('Customer Group', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['CusID'] == '') {
            ...
            $htry->execute(array('Cutomer Group', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delcusgroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Customer Group', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addbank($bankname, $bankcode, $branchname, $ifsccode, $address, $country, $state, $city, $area, $postal, $image, $order, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['BankID'] == '') {
            ...
            $resa->execute(array($bankname, $bankcode, $branchname, $ifsccode, $address, $country, $state, $city, $area, $postal, $image, $order, $status, $ip, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Bank', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['BankID'] == '') {
            ...
            $resa->execute(array($bankname, $bankcode, $branchname, $ifsccode, $address, $country, $state, $city, $area, $postal, $image, $order, $status, $ip, $_SESSION['GRUID'], $id));



            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Bank', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delbank($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Bank', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addcustomer($employee, $customer_company, $cont_person, $address1, $address2, $state, $city, $postcode, $country, $email, $website, $mobile, $phone, $gstvalue, $credit, $creditdate, $margin, $referen, $group_name, $img, $description, $status, $ip, $id) {
    ...
    if ($id == '') {
        ...
        if ($ress['CusID'] == '') {
            ...
            $resa->execute(array($employee, $customer_company, $cont_person, $address1, $address2, $state, $city, $postcode, $country, $email, $website, $mobile, $phone, $gstvalue, $credit, $creditdate, $margin, $referen, $group_name, $img, $description, $status, $ip, $_SESSION['GRUID']));

            $insert_id = $db->lastInsertId();

            $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

            $htry->execute(array('Customer', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        if ($ress['CusID'] == '') {
            ...
            $resa->execute(array($employee, $customer_company, $cont_person, $address1, $address2, $state, $city, $postcode, $country, $email, $website, $mobile, $phone, $gstvalue, $credit, $creditdate, $margin, $referen, $group_name, $img, $description, $status, $ip, $_SESSION['GRUID'], $id));
...
function delcustomer($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Customer', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
//        if ($ress['HoliID'] == '') {
        ...
        $resa->execute(array($holiday_name, $holiday_code, $description, $order, $status, $ip, $_SESSION['GRUID']));

        $insert_id = $db->lastInsertId();

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Holiday Group', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
...
//        if ($ress['HoliID'] == '') {
        ...
        $resa->execute(array($holiday_name, $holiday_code, $description, $order, $status, $ip, $_SESSION['GRUID'], $id));

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Holiday Group', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delholidaygroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Holiday Group', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
//        if ($ress['SaFID'] == '') {
        ...
        $resa->execute(array($frequent_name, $frequent_salary, $salary_period_start, $salary_period_end, $salary_date, $order, $status, $ip, $_SESSION['GRUID']));
...
//        if ($ress['SaFID'] == '') {
        ...
        $resa->execute(array($frequent_name, $frequent_salary, $salary_period_start, $salary_period_end, $salary_date, $order, $status, $ip, $_SESSION['GRUID'], $id));
...
function delsalaryfrequent($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Salary Frequent', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
//        if ($ress['WoShID'] == '') {
        ...
        $resa->execute(array($holidaygroup, $shift_code, $shift_name, $sh_str_time, $sh_end_time, $min_reg_time, $max_reg_time, $break_time, $min_break_time, $description, $weekoff, $holiday, $order, $status, $ip, $_SESSION['GRUID']));
...
//        if ($ress['WoShID'] == '') {
        ...
        $resa->execute(array($holidaygroup, $shift_code, $shift_name, $sh_str_time, $sh_end_time, $min_reg_time, $max_reg_time, $break_time, $min_break_time, $description, $weekoff, $holiday, $order, $status, $ip, $_SESSION['GRUID'], $id));
...
function delworkshift($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Work Shift', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
//        if ($ress['IncentID'] == '') {
        ...
        $resa->execute(array($group_name, $schedule, $description, $incent_value, $targettype, $item, $order, $status, $ip, $_SESSION['GRUID']));

        $insert_id = $db->lastInsertId();

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Incentive Plan', 3, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
...
//        if ($ress['IncentID'] == '') {
        ...
        $resa->execute(array($group_name, $schedule, $description, $incent_value, $targettype, $item, $order, $status, $ip, $_SESSION['GRUID'], $id));

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

        $htry->execute(array('Incentive Plan', 3, 'Update', $_SESSION['GRUID'], $ip, $id));
...
function delincentiveplan($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Incentive Plan', '3', 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function addtax($a, $b, $c, $d) {
    ...
    $resa->execute(array(strtolower(trim($a)), $b, $c, $_SESSION['GRUID'], $d));

    $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");

    $htry->execute(array('Tax', 38, 'Update', $_SESSION['GRUID'], $c, $d));
...
function addledger($name, $printname, $aliasname, $under, $undersub, $openbal, $status, $ip, $paymode, $lid) {
    ...
    if ($lid == '') {
        ...
        if ($ledgercheck > 0) {
            ...
            $resa = mysql_query("INSERT INTO `ledger`  SET `Name`='" . $name . "', `printname`='" . $printname . "',`aliasname`='" . $aliasname . "',`under` ='" . $under . "',`subledger`='" . $undersub . "',`openbal` ='" . $openbal . "',`default`= '$status',`IP`= '" . $ip . "',`Updated_By`='" . $_SESSION['GRUID'] . "',$session_value_id,`paymode`='$paymode',`status`='1' ");

            $insert_id = mysql_insert_id();

            $htry = mysql_query("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`,`Date`) VALUES ('addledger','1','Insert','" . $_SESSION['GRUID'] . "','" . $ip . "','" . $insert_id . "')");
        ...
        $resa = mysql_query("UPDATE `ledger` SET `Name`='" . $name . "',`printname`='" . $printname . "', `aliasname`='" . $aliasname . "', `under`='" . $under . "', `subledger`='" . $undersub . "',`openbal`='" . $openbal . "',`default`='$status',`IP`='" . $ip . "',`Updated_By`='" . $_SESSION['GRUID'] . "',`paymode`='" . $paymode . "',`status`='1',$session_value_id WHERE `lid`='" . $lid . "'");



        $htry = mysql_query("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES ('addledger','23','Update','" . $_SESSION['GRUID'] . "','" . $ip . "','" . $insert_id . "')");
...
function delledger($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry = mysql_query("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES ('addledger','23','Delete','" . $_SESSION['GRUID'] . "','" . $_SERVER['REMOTE_ADDR'] . "','" . $c . "')");
...
function delledgergroup($a) {
    ...
    foreach ($b as $c) {
        ...
        $resa->execute(array("ledgergroup", "23", 'Delete', $_SESSION['GRUID'], $ip, $c));
...
function additem($itemcode = '', $barcode = '', $itemname = '', $invoicename = '', $isinventory = '', $issales = '', $ispurchasable = '', $isasset = '', $logo = '', $manufacturer = '', $brand = '', $group = '', $subgroup = '', $innergroup = '', $packageuom = '', $qtyperpackage = '', $unituom = '', $issku = '', $packsku = '', $unitsku, $margin = '', $purchasepackprice = '', $purchaseunitprice = '', $mrppackprice = '', $mrpunitprice = '', $salespackprice = '', $salesunitprice = '', $tax = '', $hsnsac = '', $warehouse = '', $mintax = '', $maintainstock = '', $properties = '', $order = '', $status = '', $ip = '', $itemid = '') {
    ...
    if ($itemid == '') {
        ...
        $htry->execute(array('Item', 2, 'Insert', $_SESSION['GRUID'], $ip, $insert_id));
        ...
        $htry->execute(array('Item', 2, 'Update', $_SESSION['GRUID'], $ip, $itemid));
...
function delitem($a) {
    ...
    foreach ($b as $c) {
        ...
        $htry->execute(array('Item', 2, 'Delete', $_SESSION['GRUID'], $_SERVER['REMOTE_ADDR'], $c));
